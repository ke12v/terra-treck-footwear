...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    const savedCount = parseInt(
      localStorage.getItem("terra_treck_cart_count") || "0",
    );
  ...
  // ✅ FIX: GitHub Pages base path-safe shop scroll
  const goToShop = () => {
    const base = import.meta.env.BASE_URL; // e.g. "/terra-treck-footwear/"
    const path = window.location.pathname;

    // If not on the app base (or not on home), jump to home with #shop
    // This works for GitHub Pages where the app is under /repo-name/
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-forest-800/10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container relative flex h-20 max-w-[1440px] items-center justify-between px-4 md:px-8 mx-auto">
        {/* Search Overlay */}
        <div
          className={`absolute inset-0 bg-background z-50 transition-all duration-300 flex flex-col px-4 md:px-8 ${
        ...
        >
          <div className="flex h-20 items-center gap-4 w-full max-w-[1440px] mx-auto">
            ...
            <Button
              ...
              className="text-forest-800 hover:bg-forest-800/5"
            >
              <X className="h-6 w-6" />
            </Button>
          </div>
          ...
          {searchResults.length > 0 && (
            <div className="w-full max-w-[1440px] mx-auto bg-white border border-forest-800/5 shadow-2xl divide-y divide-forest-800/5">
              {searchResults.map((result) => (
                ...
                >
                  <span className="font-heading font-black text-forest-900 uppercase tracking-tight">
                    {result.name}
                  </span>
        ...
        <div className="flex items-center gap-2 md:gap-6">
          ...
          <Link href="/cart">
            <Button
              variant="outline"
              className="hidden md:flex gap-3 rounded-full border-forest-800 text-forest-800 hover:bg-forest-800 hover:text-white px-6 h-11 transition-all"
          ...
          <Link href="/cart" className="md:hidden">
            <Button
              variant="ghost"
              size="icon"
              className="text-forest-800 relative"
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent
              side="right"
              className="w-[300px] sm:w-[400px] bg-cream-50 border-l border-forest-800/10 p-0"
            >
              <SheetHeader className="p-6 border-b border-forest-800/10">
                <SheetTitle className="text-left font-heading font-black text-2xl uppercase tracking-tighter text-forest-900">
                  Menu
                </SheetTitle>
              </SheetHeader>
              ...
              <div className="flex flex-col py-4">
                ...
                <div className="mt-8 px-6 pt-8 border-t border-forest-800/10">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-forest-800/40 mb-4">
                    Support
                  </p>
                  <div className="flex flex-col gap-2">
                    ...
                    <Link
                      href="/wireframe"
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="text-xs font-bold text-primary mt-4 hover:underline"
                    >
                      View Site Wireframe
                    </Link>
                  </div>
